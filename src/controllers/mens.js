...
rotaMensagens.post("/mensagens", async (req, res) => {
  ...
  try {
    ...
    res.json({ mensagem: "Mensagem criada com sucesso!" });
  } catch (error) {
    console.error("Erro POST /mensagens:", error);
    res.status(500).json({ erro: "Erro ao criar mensagem" });
  }
});
...
rotaMensagens.put("/mensagens/:id", async (req, res) => {
  ...
  try {
    ...
    if (req.body.mensagem) data.mensagem = req.body.mensagem;
    if (req.body.horario) data.horario = req.body.horario;
    if (req.body.chatId) data.chatId = req.body.chatId;
